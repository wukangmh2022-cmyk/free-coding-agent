...
def agent_runtime_env(create: bool = True) -> Dict[str, str]:
    ...
    env.setdefault("AGENT_QT_APP_SCRIPT", os.path.abspath(__file__))
    env.setdefault("AGENT_QT_SCHEDULE_HELPER", os.path.join(os.path.dirname(os.path.abspath(__file__)), "scripts", "agent_qt_schedule_helper.py"))
    provider_host = env.get("AGENT_QT_AUTOMATION_HOST", "127.0.0.1").strip() or "127.0.0.1"
    provider_port = env.get("AGENT_QT_AUTOMATION_PORT", "18765").strip() or "18765"
    env.setdefault("AGENT_QT_WEB_PROVIDER_BASE_URL", f"http://{provider_host}:{provider_port}")
    return env
...
def prompt_is_windows_environment(system: str = "") -> bool:
    raw = os.environ.get("AGENT_QT_PROMPT_OS", os.environ.get("AGENT_QT_OS_NAME", "")).strip().lower()
    if raw:
        return raw.startswith("win")
    forced = os.environ.get("AGENT_QT_WINDOWS_PROMPT", "").strip().lower()
    if forced in {"1", "true", "yes", "on", "windows", "win"}:
        return True
    return (system or platform.system()) == "Windows"


def platform_specific_prompt_rules(system: str = "") -> str:
    if not prompt_is_windows_environment(system):
        return ""
    return "- Windows 如果命令代码块语言是 powershell，就只能写 PowerShell；不要混用 cmd/bat、bash 或 POSIX shell 语法。"


def web_tool_prompt_rules(system: str = "") -> str:
    download_path_note = "输出目录必须写完整绝对目录路径。"
    if prompt_is_windows_environment(system):
        download_path_note = "输出目录必须写完整绝对目录路径；Windows 路径使用 `C:\\\\...\\\\` 或 Join-Path。"
    return (
        "如果任务本质上是搜索或调研，而不是操作本地工作区，请优先使用 `web_search`。"
        "它适合找官网、入口页、列表页、详情页和公开线索，程序会把结果写回执行结果与上下文；"
        "真正执行时，在命令块里写终端扩展指令 `web_search 搜索话题`。"
        "若你已经拿到明确网址，优先在命令块里写 `web_fetch '网址'`；"
        "它底层对应 `web_fetch`，返回提炼后的 HTML 关键信息，可直接替代“先下载网页、再解析提取”的旧两步流程。"
        "若用户明确要求把网页、附件或原始 HTML 下载/保存到本地文件夹，优先使用终端扩展指令 `web_download '网址' 完整绝对输出目录`，"
        f"{download_path_note}不要自己写 requests/curl/wget 下载脚本；"
        "`web_download` 会保存原始字节，HTML 另生成 UTF-8 正文副本，避免 `requests.text` 编码误判导致乱码。"
        "除非用户已经明确给出网址，或上一轮 `web_search` / 已抓到的页面里已经出现可访问链接，否则不要自己补路径、猜列表页 URL、逐个试错网址；"
        "`web_fetch` 默认依赖 `web_search` 找到或已验证可访问的网址。"
        "只要上一轮输出里已经出现具体 URL，后续 `web_fetch` 必须逐字复用该 URL，禁止删路径、补路径、把相对路径脑补成更短地址，或根据标题摘要自行改写 URL。"
        "不要为了普通搜索或调研而生成本地 curl/wget/python 抓取脚本。"
        "例外：用户明确要求“扒下来/抓取/爬取/下载到文件夹/全部公告/全部页面/官网列表”，或目标数据只存在某个站点/当前工作区/本机文件里时，应优先写真实脚本访问目标站点、处理分页、进入详情页并保存结果，不要只依赖搜索结果摘要。"
    )
...
def runtime_environment() -> Dict[str, str]:
    ...
    return {
        ...
        "platform_specific_rules": platform_specific_prompt_rules(system),
        "web_tool_rules": web_tool_prompt_rules(system),
    }
...
TERMINAL_EXTENSION_COMMAND_SPECS = (
    ...
    {
        "name": "web_search",
        "prefix_re": re.compile(r"^(?:web_search(?:\s|$)|AGENT_WEB_RESEARCH(?:\s|[:：]|$))", re.I),
        "complete_re": re.compile(r"^(?:web_search\s+.+|AGENT_WEB_RESEARCH(?:\s*[:：]\s*|\s+).+)$", re.I),
        "internal_prefixes": ("AGENT_WEB_RESEARCH",),
        "usage": "`web_search 搜索话题`",
    },
    {
        "name": "web_fetch",
        "prefix_re": re.compile(r"^(?:web_fetch(?:\s|$)|AGENT_WEB_FETCH(?:\s|[:：]|$))", re.I),
        "complete_re": re.compile(r"^(?:web_fetch\s+.+|AGENT_WEB_FETCH(?:\s*[:：]\s*|\s+).+)$", re.I),
        "internal_prefixes": ("AGENT_WEB_FETCH",),
        "usage": "`web_fetch '网址'`",
    },
    {
        "name": "skill list",
...
def normalize_terminal_extension_directive(line: str) -> List[str]:
    ...
    web_research_match = re.match(r"^web_search\s+(.+)$", stripped, re.I)
    ...
    web_fetch_match = re.match(r"^web_fetch\s+(.+)$", stripped, re.I)
    if web_fetch_match:
        url = terminal_extension_payload_text(web_fetch_match.group(1) or "")
        if url:
            return [f"AGENT_WEB_FETCH: {url}"]
    ...
    if send_match:
        ...
        targets = [part.strip().strip("\"'") for part in re.split(r"\s*,\s*", tail) if part.strip()]
        return [f"AGENT_WECHAT_SEND_FILE: {target}" for target in targets]
    for name in ("AGENT_WECHAT_CREATE_SCHEDULE", "AGENT_WECHAT_SCHEDULE_ACTION", "AGENT_WEB_RESEARCH", "AGENT_WEB_FETCH", "AGENT_SKILL_LIST"):
...
def extract_web_fetch_urls(text: str) -> List[str]:
    urls: List[str] = []
    for line in str(text or "").splitlines():
        normalized = normalize_terminal_extension_directive(line)
        for directive in normalized:
            upper = directive.upper()
            if not upper.startswith("AGENT_WEB_FETCH:") and not upper.startswith("AGENT_WEB_FETCH："):
                continue
            _, _, tail = directive.partition(":" if ":" in directive else "：")
            payload = terminal_extension_payload_text(tail)
            if not payload:
                continue
...
def summarize_web_research_response(payload: dict) -> str:
    if isinstance(payload, dict) and (
        payload.get("object") == "web_search.result"
        or isinstance(payload.get("results"), list)
    ):
        query = str(payload.get("query") or "").strip()
        results = payload.get("results") if isinstance(payload.get("results"), list) else []
        lines = [f"网页搜索：{query}" if query else "网页搜索结果："]
        if not results:
...
def terminal_extension_directives_from_text(text: str) -> List[str]:
    ...
    for command_text, _command_lang in command_blocks_from_text(text):
        if not command_block_is_pure_terminal_extensions(command_text):
            continue
...
def command_block_is_pure_terminal_extensions(command_text: str) -> bool:
    has_directive = False
    for raw_line in str(command_text or "").splitlines():
        line = raw_line.strip()
...
def command_contains_shell_terminal_extensions(command_text: str) -> bool:
    return bool(re.search(r"(^|[\s;&|()])(?:web_search|web_fetch|web_download|schedule)\b", str(command_text or ""), re.I))


def posix_shell_terminal_extension_helpers() -> str:
    return r"""
_agent_qt_web_provider_url() {
  if [ -n "${AGENT_QT_WEB_PROVIDER_BASE_URL:-}" ]; then
    printf '%s\n' "$AGENT_QT_WEB_PROVIDER_BASE_URL"
  else
    printf 'http://%s:%s\n' "${AGENT_QT_AUTOMATION_HOST:-127.0.0.1}" "${AGENT_QT_AUTOMATION_PORT:-18765}"
  fi
}
...
web_fetch() {
  local _url="${1:-}"
  if [ -z "$_url" ]; then
    echo "web_fetch: missing url" >&2
    return 2
  fi
  AGENT_QT_WEB_PROVIDER_URL="$(_agent_qt_web_provider_url)" \
  AGENT_QT_WEB_FETCH_URL="$_url" \
  "${AGENT_QT_RUNTIME_PYTHON:-python3}" - <<'PY'
import json, os, sys, urllib.error, urllib.request
url = os.environ.get("AGENT_QT_WEB_FETCH_URL", "").strip()
base = os.environ.get("AGENT_QT_WEB_PROVIDER_URL", "").strip()
payload = {"url": url, "include_html": False}
req = urllib.request.Request(base + "/v1/web-fetch", data=json.dumps(payload, ensure_ascii=False).encode("utf-8"), headers={"Content-Type": "application/json"}, method="POST")
...
for attempt in range(1, 4):
    try:
        ...
        last_error = body or f"HTTP {exc.code}: {exc.reason}"
        if 400 <= exc.code < 500:
        ...
        last_error = str(exc)
    if attempt < 3:
        print(f"web_fetch retry {attempt}/3: {last_error}", file=sys.stderr)
        import time
        time.sleep(1.5 * attempt)
else:
    print(last_error or "web_fetch failed", file=sys.stderr)
    raise SystemExit(1)
title = str(data.get("title") or "").strip()
page_url = str(data.get("url") or url).strip()
status = data.get("status")
content_type = str(data.get("content_type") or "").strip()
content = str(data.get("content") or "").strip()
header = " | ".join(part for part in (title, page_url) if part)
if header:
    print(header)
meta = []
...
web_download() {
  ...
  local _output="${2:-}"
  if [ -z "$_url" ]; then
    echo "web_download: missing url" >&2
    return 2
  fi
  AGENT_QT_WEB_DOWNLOAD_URL="$_url" \
  AGENT_QT_WEB_DOWNLOAD_OUTPUT="$_output" \
  "${AGENT_QT_RUNTIME_PYTHON:-python3}" - <<'PY'
import os, re, sys, mimetypes, urllib.parse, urllib.request
from pathlib import Path
url = os.environ.get("AGENT_QT_WEB_DOWNLOAD_URL", "").strip()
output = os.environ.get("AGENT_QT_WEB_DOWNLOAD_OUTPUT", "").strip()
headers = {"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X) AppleWebKit/537.36 AgentQtWebDownload/1.0"}
...
try:
    ...
    print(f"web_download failed: {exc}", file=sys.stderr)
    raise SystemExit(1)
...
def filename_from_content_disposition(value: str) -> str:
    ...
    if m:
        return urllib.parse.unquote((m.group(1) or m.group(2) or "").strip().strip('"'))
    return ""

def safe_name_from_url(value: str) -> str:
    parsed = urllib.parse.urlparse(value)
    name = filename_from_content_disposition(content_disposition)
    if not name:
        name = Path(urllib.parse.unquote(parsed.path or "")).name
    if not name:
        ext = mimetypes.guess_extension((content_type.split(";", 1)[0] or "").strip()) or ".html"
        name = "download" + ext
    if name.lower() in {"download", "download.jsp", "download.php", "download.aspx"}:
        ext = mimetypes.guess_extension((content_type.split(";", 1)[0] or "").strip()) or ""
...
if output:
    ...
    if str(output).endswith(("/", os.sep)) or (out_path.exists() and out_path.is_dir()):
        out_path = out_path / safe_name_from_url(final_url)
else:
    out_path = Path.cwd() / safe_name_from_url(final_url)
out_path.parent.mkdir(parents=True, exist_ok=True)
is_html = ("html" in content_type.lower()) or out_path.suffix.lower() in {".html", ".htm", ".xhtml", ""}
preview_text = ""
if is_html:
    for _enc in ("utf-8-sig", "utf-8", "gb18030", "gbk"):
        try:
            preview_text = body.decode(_enc)
            break
        except Exception:
            continue
    if not preview_text:
        preview_text = body.decode("utf-8", "replace")
captcha_hit = bool(
    is_html
    and re.search(r"(请输入验证码|验证码下载附件|附件下载|createimage\.jsp|codeValue|changeCodeImg)", preview_text, re.I)
    and re.search(r"(验证码|codeValue|createimage\.jsp)", preview_text, re.I)
)
if captcha_hit:
    captcha_path = out_path.with_suffix(out_path.suffix + ".captcha.html") if out_path.suffix else out_path.with_name(out_path.name + ".captcha.html")
    captcha_path.write_bytes(body)
    img_match = re.search(r"<img[^>]+(?:id=['\"]codeimg['\"][^>]*src|src)=['\"]([^'\"]*createimage\.jsp[^'\"]*)['\"]", preview_text, re.I)
    img_url = urllib.parse.urljoin(final_url, img_match.group(1)) if img_match else ""
    print(f"网页下载：{final_url}")
    print("下载受阻：页面要求输入验证码，未下载到真实附件。")
    print(f"验证码页面：{captcha_path}")
    if img_url:
        print(f"验证码图片：{img_url}")
    print("建议：保留该附件 URL 清单，交给用户在浏览器中手动输入验证码下载，或后续接入浏览器自动化人工确认流程。")
    raise SystemExit(3)
out_path.write_bytes(body)
print(f"网页下载：{final_url}")
print(f"原始文件：{out_path}")
...
def sniff_encoding(data: bytes, ctype: str) -> str:
...
encodings = []
for enc in (sniff_encoding(body, content_type), "utf-8-sig", "utf-8", "gb18030", "gbk"):
    if enc and enc.lower() not in [item.lower() for item in encodings]:
...
if text is None:
    ...
    used = "utf-8-replace"
utf8_path = out_path.with_suffix(out_path.suffix + ".utf8.html") if out_path.suffix else out_path.with_name(out_path.name + ".utf8.html")
utf8_path.write_text(text, encoding="utf-8")
print(f"UTF-8正文：{utf8_path}")
print(f"解码：{used}")
m = re.search(r"<title[^>]*>(.*?)</title>", text, re.I | re.S)
if m:
    title = re.sub(r"\\s+", " ", m.group(1)).strip()
    if title:
        print(f"标题：{title[:160]}")
PY
}
...
web_search() {
  local _query="$*"
  if [ -z "$_query" ]; then
    echo "web_search: missing query" >&2
    return 2
  fi
  AGENT_QT_WEB_PROVIDER_URL="$(_agent_qt_web_provider_url)" \
  AGENT_QT_WEB_SEARCH_QUERY="$_query" \
  "${AGENT_QT_RUNTIME_PYTHON:-python3}" - <<'PY'
import json, os, sys, urllib.error, urllib.request
query = os.environ.get("AGENT_QT_WEB_SEARCH_QUERY", "").strip()
base = os.environ.get("AGENT_QT_WEB_PROVIDER_URL", "").strip()
payload = {"query": query, "max_results": 10}
req = urllib.request.Request(base + "/v1/web-search", data=json.dumps(payload, ensure_ascii=False).encode("utf-8"), headers={"Content-Type": "application/json"}, method="POST")
opener = urllib.request.build_opener(urllib.request.ProxyHandler({}))
try:
    with opener.open(req, timeout=900) as resp:
        data = json.loads(resp.read().decode("utf-8", "ignore"))
...
schedule() {
  ...
  shift || true
  if [ -z "$_action" ]; then
    echo "schedule: missing action" >&2
    return 2
  fi
  local _helper="${AGENT_QT_SCHEDULE_HELPER:-}"
  if [ -z "$_helper" ] || [ ! -f "$_helper" ]; then
    echo "schedule: helper script missing" >&2
    return 2
  fi
  "${AGENT_QT_RUNTIME_PYTHON:-python3}" "$_helper" "${AGENT_QT_PROJECT_ROOT:-$PWD}" "$_action" "$*"
}
"""


def powershell_terminal_extension_helpers() -> str:
    return r"""
function _AgentQtWebProviderUrl {
  if ($env:AGENT_QT_WEB_PROVIDER_BASE_URL) { return $env:AGENT_QT_WEB_PROVIDER_BASE_URL }
  $hostName = if ($env:AGENT_QT_AUTOMATION_HOST) { $env:AGENT_QT_AUTOMATION_HOST } else { '127.0.0.1' }
  $port = if ($env:AGENT_QT_AUTOMATION_PORT) { $env:AGENT_QT_AUTOMATION_PORT } else { '18765' }
  return "http://${hostName}:${port}"
}

function _AgentQtPython {
  if ($env:AGENT_QT_RUNTIME_PYTHON) { return $env:AGENT_QT_RUNTIME_PYTHON }
  return 'python'
}

function web_fetch {
  param([Parameter(ValueFromRemainingArguments=$true)][string[]]$Args)
  $url = ($Args -join ' ').Trim().Trim("'", '"')
  if (-not $url) { Write-Error 'web_fetch: missing url'; return }
  $env:AGENT_QT_WEB_PROVIDER_URL = _AgentQtWebProviderUrl
  $env:AGENT_QT_WEB_FETCH_URL = $url
  $py = _AgentQtPython
@'
import json, os, sys, urllib.error, urllib.request
url = os.environ.get("AGENT_QT_WEB_FETCH_URL", "").strip()
...
if content:
    print(content[:4000])
'@ | & $py -
  if ($LASTEXITCODE -ne 0) { throw "web_fetch failed with exit code $LASTEXITCODE" }
}

function web_search {
  param([Parameter(ValueFromRemainingArguments=$true)][string[]]$Args)
  $query = ($Args -join ' ').Trim()
  if (-not $query) { Write-Error 'web_search: missing query'; return }
  $env:AGENT_QT_WEB_PROVIDER_URL = _AgentQtWebProviderUrl
  $env:AGENT_QT_WEB_SEARCH_QUERY = $query
  $py = _AgentQtPython
@'
import json, os, sys, urllib.error, urllib.request
query = os.environ.get("AGENT_QT_WEB_SEARCH_QUERY", "").strip()
...
for index, item in enumerate(results[:10], 1):
    ...
    if content:
        print(f"   {content}")
'@ | & $py -
  if ($LASTEXITCODE -ne 0) { throw "web_search failed with exit code $LASTEXITCODE" }
}

function web_download {
  param([string]$Url, [string]$Output = '')
  $url = ($Url -as [string]).Trim().Trim("'", '"')
  if (-not $url) { Write-Error 'web_download: missing url'; return }
  $env:AGENT_QT_WEB_DOWNLOAD_URL = $url
  $env:AGENT_QT_WEB_DOWNLOAD_OUTPUT = ($Output -as [string]).Trim().Trim("'", '"')
  $py = _AgentQtPython
@'
import os, re, sys, mimetypes, urllib.parse, urllib.request
from pathlib import Path
url = os.environ.get("AGENT_QT_WEB_DOWNLOAD_URL", "").strip()
output = os.environ.get("AGENT_QT_WEB_DOWNLOAD_OUTPUT", "").strip()
headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 AgentQtWebDownload/1.0"}
req = urllib.request.Request(url, headers=headers)
try:
    with urllib.request.urlopen(req, timeout=120) as resp:
        body = resp.read()
...
if output:
    out_path = Path(output).expanduser()
    if str(output).endswith(("/", "\\")) or (out_path.exists() and out_path.is_dir()):
        out_path = out_path / safe_name_from_url(final_url)
else:
    out_path = Path.cwd() / safe_name_from_url(final_url)
out_path.parent.mkdir(parents=True, exist_ok=True)
...
if is_html:
    ...
    if not preview_text:
        preview_text = body.decode("utf-8", "replace")
captcha_hit = bool(is_html and re.search(r"(请输入验证码|验证码下载附件|附件下载|createimage\.jsp|codeValue|changeCodeImg)", preview_text, re.I) and re.search(r"(验证码|codeValue|createimage\.jsp)", preview_text, re.I))
if captcha_hit:
    captcha_path = out_path.with_suffix(out_path.suffix + ".captcha.html") if out_path.suffix else out_path.with_name(out_path.name + ".captcha.html")
    captcha_path.write_bytes(body)
    img_match = re.search(r"<img[^>]+(?:id=['\"]codeimg['\"][^>]*src|src)=['\"]([^'\"]*createimage\.jsp[^'\"]*)['\"]", preview_text, re.I)
...
if m:
    ...
    if title:
        print(f"标题：{title[:160]}")
'@ | & $py -
  if ($LASTEXITCODE -ne 0) { throw "web_download failed with exit code $LASTEXITCODE" }
}

function schedule {
  param([string]$Action, [Parameter(ValueFromRemainingArguments=$true)][string[]]$Rest)
  if (-not $Action) { Write-Error 'schedule: missing action'; return }
  $helper = $env:AGENT_QT_SCHEDULE_HELPER
  if (-not $helper -or -not (Test-Path -LiteralPath $helper)) { Write-Error 'schedule: helper script missing'; return }
  $root = if ($env:AGENT_QT_PROJECT_ROOT) { $env:AGENT_QT_PROJECT_ROOT } else { (Get-Location).Path }
  $payload = ($Rest -join ' ')
  & (_AgentQtPython) $helper $root $Action $payload
  if ($LASTEXITCODE -ne 0) { throw "schedule failed with exit code $LASTEXITCODE" }
}
"""
...
def summarize_web_fetch_response(payload: dict) -> str:
    ...
    lines: List[str] = []
    header_parts = [part for part in [title, url] if part]
    if header_parts:
            ...
            if stream is not None:
                while True:
                    ...
                    if isinstance(chunk, bytes):
                        chunk = decode_process_output(chunk)
                    f.write(str(chunk))
                    f.flush()
...
def run_shell_command_capture(
    ...
    script_path = ""
    effective_cmd = cmd
    if platform.system() != "Windows" and command_contains_shell_terminal_extensions(effective_cmd):
        effective_cmd = posix_shell_terminal_extension_helpers().strip() + "\n\n" + effective_cmd
    if platform.system() == "Windows" and is_powershell_command(cmd):
        powershell_body = strip_shell_command_marker(effective_cmd)
        if command_contains_shell_terminal_extensions(powershell_body):
            effective_cmd = POWERSHELL_COMMAND_PREFIX + powershell_terminal_extension_helpers().strip() + "\n\n" + powershell_body
        script_path = write_temp_shell_script(effective_cmd)
    elif should_use_temp_shell_script(effective_cmd):
        script_path = write_temp_shell_script(effective_cmd)
    shell, args = shell_launch_for_command(effective_cmd, script_path=script_path)
    terminal_id = ""
    log_path = ""
    if project_root:
    ...
    if project_root:
        env["AGENT_QT_PROJECT_ROOT"] = project_root
    ...
    try:
        if stream_callback is not None and process.stdout is not None:
            captured_parts: List[bytes] = []
            ...
            while True:
                ...
                if remaining <= 0:
                    ...
                    remove_temp_script_later(script_path)
                    raise subprocess.TimeoutExpired([shell, *args], timeout, output=decode_process_output(b"".join(captured_parts)))
                try:
                    item = output_queue.get(timeout=min(0.1, max(0.01, remaining)))
                except queue.Empty:
                    continue
                if item is None:
                    reader_done = True
                ...
                try:
                    stream_callback(decode_process_output(b"".join(captured_parts)))
                except Exception:
                    pass
            stdout = decode_process_output(b"".join(captured_parts))
            process.wait(timeout=max(1, int(max(1, deadline - time.monotonic()))))
            remove_temp_script_later(script_path)
            return subprocess.CompletedProcess([shell, *args], process.returncode, stdout or "", "")
        stdout, _stderr = process.communicate(timeout=timeout)
        remove_temp_script_later(script_path)
        return subprocess.CompletedProcess([shell, *args], process.returncode, decode_process_output(stdout or b""), "")
...
def format_terminal_launch_summary(launches: List[Dict[str, object]]) -> str:
    ...
    lines.append("查看该后台终端输出：sleep 15 && curl -s 'http://127.0.0.1:8798/terminallogs?pid=xxx'，把 xxx 换成上面的 pid；不要刚后台化就立刻查询。")
    return "\n".join(lines)
...
def provider_retry_status_message(path: str, attempt: int, attempts: int, detail: str = "") -> str:
    target = "网页搜索" if any(part in str(path or "") for part in ("/v1/responses", "/v1/web-search")) else "AI 回复"
...
def build_automation_feedback_prompt(
    ...
    *,
    ...
    if "web_search " in log_text.lower() or "AGENT_WEB_RESEARCH" in log_text:
        web_research_followup_note = (
            "\n- 如果上一轮本地执行结果里已经出现 `网页搜索：` 开头的结果，说明 `web_search` 已经真的执行完了。"
            " 下一轮不要再次输出 `web_search` 命令，也不要解释网页搜索工具协议；"
            ...
            "如果搜索结果或上一轮网页抓取结果里已经出现具体 URL，后续 `web_fetch` 必须逐字复用那个 URL；不要缩短、补路径、猜测同站其他地址，也不要把标题里的栏目名直接改写成路径。"
    ...
    web_fetch_followup_note = ""
    if "网页抓取：" in log_text:
        web_fetch_followup_note = (
            "\n- 如果上一轮本地执行结果里已经出现 `网页抓取：`，后续需要继续 `web_fetch` 时必须优先复用上一轮结果里已经验证成功的 exact URL。"
            "若某个 URL 已经返回 404/Not Found，就不要再次抓取同一个错误地址；应回到上一轮已出现的具体链接，或先重新 `web_search`/抓首页关键链接，再从结果里逐字复制新的 exact URL。"
        )
...
- 命令块内只能写真实要执行的 shell 代码，或 `wx send_file 路径`、`schedule create/list/delete/update`、`web_search 搜索话题`、`web_fetch '网址'`、`web_download '网址' 完整绝对输出目录`；不要把上一轮执行结果、文件变更、结论或完成标记写进命令块。
...
- {web_tool_prompt_rules()}
- 后台安装/构建/拉取不要当作完成；启动常驻命令时不要加 `&`/`nohup`，等执行结果给出 `Terminal processes:` 摘要后，不要下一轮立刻秒查；若需要查看进度，使用 `sleep 15 && curl -s 'http://127.0.0.1:8798/terminallogs?pid=xxx'` 查询控制台输出，等待秒数按任务调整。
- 对下载、联网 HTTP 调用、安装、构建、长时间生成等任务，如果已经转入后台或暂时无输出，不要立刻换方案，也不要下一轮立刻查空气。优先先等待一小段合理时间，并把等待与日志查询合并到同一个命令块；若日志显示仍在运行，除非用户催促或有明确错误，不要连续多轮只查同一个后台日志。
...
class ExecuteWorker(QThread):
    ...
    replace_output_signal = Signal(int, str)
    ...
    def run(self):
        ...
        for i, cmd in enumerate(self.commands, 1):
            ...
            if self.isInterruptionRequested():
                set_output(output_index, f"[{i}] ⏹️ 已停止后续命令")
                break
            cmd = strip_provider_ui_artifacts_from_command(cmd)
            if not cmd.strip():
                set_output(output_index, f"[{i}] ⚠️ 跳过空命令")
                continue
            ...
            if target is not None:
                if os.path.isdir(target):
                    cwd = target
                    set_output(output_index, f"[{i}] 📂 {display_cmd}\n✅ → {cwd}")
                else:
                    set_output(output_index, f"[{i}] 📂 {display_cmd}\n⚠️ 目录不存在")
                continue
            safety_rejection = command_safety_rejection(cmd)
            if safety_rejection:
                set_output(output_index, f"[{i}] ⛔ 拒绝: {display_cmd}\n{safety_rejection}")
                continue
            ...
            if is_interactive_shell_command(cmd):
                set_output(output_index, f"[{i}] ⚠️ 跳过交互式 Shell: {display_cmd}")
                continue
            ...
            if syntax_error:
                ...
                continue
            if is_long_running(cmd):
                set_output(output_index, f"[{i}] 🔵 后台: {display_cmd}")
                self.long_running_signal.emit(cmd, cwd, display_cmd.splitlines()[0][:40], "long_running_pattern", command_kind(cmd))
                continue
            try:
                streaming_shell_helpers = command_contains_shell_terminal_extensions(cmd)
                if streaming_shell_helpers:
                    def _stream_update(current_text: str, idx=output_index, display=display_cmd):
                        body = current_text.strip()
                        set_output(idx, f"[{idx + 1}] 💻 {display}\n📤 {body or '(无输出)'}")
                else:
                    _stream_update = None
                r = run_shell_command_capture(
                    ...
                    stream_callback=_stream_update,
                )
                out = r.stdout.strip()
                if r.stderr.strip():
                    out += "\n" + r.stderr.strip()
                ...
                if is_interactive_shell_command(cmd):
                    set_output(output_index, f"[{i}] ⚠️ 交互式 Shell 已超时，未创建后台终端: {display_cmd}")
                else:
                    set_output(output_index, f"[{i}] ⏱️ 超时 → 后台: {display_cmd}")
                    self.long_running_signal.emit(cmd, cwd, display_cmd.splitlines()[0][:40], "timeout", command_kind(cmd))
            except Exception as e:
                set_output(output_index, f"[{i}] ❌ {e}")
...
class AutomationProviderManager:
    ...
    def provider_env(self) -> Dict[str, str]:
        env = os.environ.copy()
        use_proxy = use_system_proxy_setting()
        if not use_proxy:
...
"""
    ...
    def web_research(
        ...
        *,
        ...
        progress(f"准备网页搜索请求：max_results={max(1, int(max_results or 10))}")
        payload = self.request_json(
            "POST",
            "/v1/web-search",
            {
...
class ProviderProxyModeWorker(QThread):
    finished_signal = Signal(bool, str)

    def __init__(self, base_url: str, use_system_proxy: bool):
        super().__init__()
    ...
    def run(self):
        if not self.base_url:
            self.finished_signal.emit(False, "Provider 地址为空，代理状态将在下次启动时生效。")
            return
        payload = {"use_system_proxy": self.use_system_proxy, "proxy_env": self.proxy_env}
        try:
            data = json.dumps(payload, ensure_ascii=False).encode("utf-8")
            request = urllib.request.Request(
                f"{self.base_url}/debug/proxy-mode",
                data=data,
            ...
            if result.get("status") == "ok":
                mode = "开启" if self.use_system_proxy else "关闭"
                self.finished_signal.emit(True, f"Provider 代理模式已同步为{mode}。")
                return
            self.finished_signal.emit(False, f"Provider 代理同步返回异常：{result}")
        except Exception as exc:
            self.finished_signal.emit(False, f"Provider 代理状态将在下次启动时生效：{exc}")


class DiagnosticsLoadWorker(QThread):
    ...
    def run(self):
        ...
        errors: List[str] = []
        try:
            status_text = self.manager.status_text()
        except Exception as exc:
            errors.append(f"状态加载失败：{exc}")
            status_text = errors[-1]
        try:
            ...
            payload = {
                ...
                "web_search_url": f"{base_url}/v1/web-search",
                ...
                "output_protocols": {
                    "plain": {
                        ...
                        "purpose": "Agent Qt 本地 runner：自然 Markdown/bash 输出，不使用 JSON/tool_calls。",
                    },
                    "openai": {
                        "endpoints": ["/v1/chat/completions", "/v1/responses"],
                        "purpose": "OpenAI-compatible agent 接入：JSON/tool_calls 或 Responses web_search 工具链。",
                    },
                    "anthropic": {
                        "endpoint": "/v1/messages",
                        "purpose": "Anthropic-compatible agent 接入：tool_use/messages 格式。",
                    },
                },
                "web_ui_search_enabled": False,
                "status": "running" if self.manager.health() else "stopped",
            }
        except Exception as exc:
            errors.append(f"Provider 信息加载失败：{exc}")
        try:
            log_text = self.manager.log_tail(12000) or "日志为空。"
        except Exception as exc:
            errors.append(f"日志加载失败：{exc}")
            log_text = errors[-1]
        self.finished_signal.emit({
...
def terminal_extension_command_for_log(directive: str) -> str:
    ...
    if upper.startswith("AGENT_WEB_RESEARCH:") or upper.startswith("AGENT_WEB_RESEARCH："):
        _, _, tail = stripped.partition(":" if ":" in stripped else "：")
        return "web_search " + tail.strip()
    if upper.startswith("AGENT_WEB_FETCH:") or upper.startswith("AGENT_WEB_FETCH："):
        _, _, tail = stripped.partition(":" if ":" in stripped else "：")
        return "web_fetch " + tail.strip()
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                ...
                if path in {"/threads/new", "/conversation/new"}:
                    ...
                    self._send(200, bridge.dispatch(payload, wait=True))
                    return
                if path in {"/project/open", "/workspace/open"}:
                    payload["action"] = "open_project"
                    self._send(200, bridge.dispatch(payload, wait=True))
                    return
                self._send(404, {"ok": False, "error": "not_found"})
...
class ExecutionLogPanel(QFrame):
    ...
    def update_content(self, text: str):
        next_content = mask_low_value_context_markers_for_display(text)
        if next_content.startswith(self.content):
            delta = next_content[len(self.content):]
            if delta:
                cursor = QTextCursor(self.editor.document())
                cursor.movePosition(QTextCursor.MoveOperation.End)
                cursor.insertText(delta)
        else:
            self.editor.setPlainText(next_content)
...
class ChatPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.provider_proxy_mode_workers: List[ProviderProxyModeWorker] = []
    ...
    def handle_wechat_bridge_request(self, payload: dict):
        ...
        try:
            ...
            if action in {"threads", "conversations"}:
                ...
                return
            if action in {"open_project", "set_project", "workspace_open"}:
                path = str((payload or {}).get("path") or (payload or {}).get("project_root") or "").strip()
                if not path:
                    raise RuntimeError("缺少工作区路径。")
                path = os.path.abspath(os.path.expanduser(path))
                if not os.path.isdir(path):
                    raise RuntimeError(f"工作区不存在：{path}")
                if self.is_automation_busy() or self.is_execution_running():
                    raise RuntimeError("当前还有任务在运行，无法切换工作区。")
                self.set_project(path)
                self.wechat_bridge.finish_request(request_id, {"ok": True, "project_root": self.project_root, "active_thread_id": self.thread_id})
                return
...
```"""
    ...
    def show_preferences_dialog(self):
        ...
        def on_proxy_toggled(enabled: bool):
            ...
            worker = ProviderProxyModeWorker(self.automation_manager.base_url, enabled)
            self.provider_proxy_mode_workers.append(worker)

            def on_proxy_sync_finished(ok: bool, message: str, current_worker=worker):
                if current_worker in self.provider_proxy_mode_workers:
                    self.provider_proxy_mode_workers.remove(current_worker)
                if message:
                    self.add_status_bubble(message if ok else f"警告：{message}")
                current_worker.deleteLater()

            worker.finished_signal.connect(on_proxy_sync_finished)
            worker.start()
    ...
    def show_settings_menu(self):
        ...
        automation_menu.addSeparator()
        diagnostics_action = QAction("高级诊断", self)
    ...
    def provider_info_payload(self) -> Dict[str, object]:
        return {
            ...
            "web_search_url": f"{self.automation_manager.base_url}/v1/web-search",
            "messages_url": f"{self.automation_manager.base_url}/v1/messages",
            "models_url": f"{self.automation_manager.base_url}/v1/models",
            "status_url": f"{self.automation_manager.base_url}/status",
            "api_key": "not-required",
    ...
    def _diagnostics_button(self, text: str, callback) -> QPushButton:
        button = QPushButton(text, cursor=Qt.CursorShape.PointingHandCursor)
    ...
    def _diagnostics_text_box(self, text: str, *, height: int | None = None) -> QPlainTextEdit:
    ...
    def _provider_info_tabs(self, payload: Dict[str, object], parent: QWidget | None = None) -> QTabWidget:
        def endpoint_row(label: str, value: str) -> QWidget:
            row = QWidget(parent)
            ...
            title.setStyleSheet(f"color: {COLORS['text_secondary']}; background: transparent; font-size: 12px; font-weight: 900;")
            ...
            """)
            copy_btn = self._diagnostics_button("复制", lambda value=value: QApplication.clipboard().setText(value))
        ...
        def protocol_tab(rows: list[tuple[str, str]], sample: dict[str, object] | str, note: str) -> QWidget:
            tab = QWidget(parent)
            ...
            note_label.setStyleSheet(f"color: {COLORS['text_secondary']}; background: transparent; font-size: 12px; font-weight: 800;")
            tab_layout.addWidget(note_label)
            for label, value in rows:
                tab_layout.addWidget(endpoint_row(label, value))
            sample_text = sample if isinstance(sample, str) else json.dumps(sample, ensure_ascii=False, indent=2)
            sample_box = self._diagnostics_text_box(sample_text, height=185)
            tab_layout.addWidget(sample_box)
            actions = QHBoxLayout()
            actions.addWidget(self._diagnostics_button("复制示例", lambda box=sample_box: QApplication.clipboard().setText(box.toPlainText())))
            actions.addStretch()
            tab_layout.addLayout(actions)
            return tab

        base_url = str(payload.get("api_url") or self.automation_manager.base_url)
        model = str(payload.get("model") or self.effective_automation_model())
        chat_url = str(payload.get("chat_completions_url") or f"{base_url}/v1/chat/completions")
        responses_url = str(payload.get("responses_url") or f"{base_url}/v1/responses")
        web_search_url = str(payload.get("web_search_url") or f"{base_url}/v1/web-search")
        messages_url = str(payload.get("messages_url") or f"{base_url}/v1/messages")
        status_url = str(payload.get("status_url") or f"{base_url}/status")
        ...
        tabs.addTab(
            protocol_tab(
                [
                    ...
                    ("Direct Search", web_search_url),
                    ("Models", str(payload.get("models_url") or f"{base_url}/v1/models")),
                ],
                {
                    "model": model,
                ...
                "OpenAI-compatible 接入。默认 Chat Completions 使用 openai 输出协议；需要工具/网页搜索时可走 Responses。",
            ),
            "OpenAI",
        )
        tabs.addTab(
            protocol_tab(
                ...
                "Anthropic-compatible 接入。使用 /v1/messages，provider 内部会走 anthropic 输出范式。",
            ),
            "Anthropic",
        )
        tabs.addTab(
            protocol_tab(
                ...
                ],
                {
                    ...
                    "output_protocol": "plain",
                    "extra_body": {"output_protocol": "plain"},
                    "messages": [{"role": "user", "content": "在本地项目里运行测试"}],
                },
                "Agent Qt 内部 runner 使用的 plain 协议。输出是自然 Markdown/bash，不是 JSON/tool_calls。",
            ),
            "Plain Runner",
        )
        tabs.addTab(
            protocol_tab(
                [
                    ("Status", status_url),
                    ("API Key", str(payload.get("api_key") or "not-required")),
                ],
                json.dumps(payload, ensure_ascii=False, indent=2),
                "原始发现信息，适合复制给其它 agent 或排查 provider 状态。",
            ),
            "Raw JSON",
        )
        return tabs

    def show_advanced_diagnostics_dialog(self, initial_tab: str = "status"):
        dialog = QDialog(self)
        dialog.setWindowTitle("高级诊断")
        dialog.setMinimumSize(720, 520)
        layout = QVBoxLayout(dialog)
        layout.setContentsMargins(14, 14, 14, 14)
        layout.setSpacing(10)

        title = QLabel("自动化插件高级诊断")
        title.setStyleSheet(f"color: {COLORS['text']}; background: transparent; font-size: 16px; font-weight: 900;")
        layout.addWidget(title)
        ...
        status_tab = QWidget(dialog)
        status_layout = QVBoxLayout(status_tab)
        status_layout.setContentsMargins(12, 12, 12, 12)
        status_layout.setSpacing(10)
        status_text = self._diagnostics_text_box("状态加载中...")
        status_layout.addWidget(status_text, 1)
        status_row = QHBoxLayout()
        status_row.addWidget(self._diagnostics_button("刷新状态", lambda: status_text.setPlainText(self.automation_manager.status_text())))
        status_row.addWidget(self._diagnostics_button("复制状态", lambda: QApplication.clipboard().setText(status_text.toPlainText())))
        ...
        provider_placeholder = self._diagnostics_text_box("Provider 信息加载中...", height=320)
        provider_layout.addWidget(provider_placeholder, 1)
        provider_row = QHBoxLayout()
        provider_row.addWidget(self._diagnostics_button("复制 Provider JSON", lambda: QApplication.clipboard().setText(json.dumps(self.provider_info_payload(), ensure_ascii=False, indent=2))))
        provider_row.addStretch()
        provider_layout.addLayout(provider_row)
        tabs.addTab(provider_tab, "Provider")

        log_tab = QWidget(dialog)
        log_layout = QVBoxLayout(log_tab)
        log_layout.setContentsMargins(12, 12, 12, 12)
        log_layout.setSpacing(10)
        log_hint = QLabel(f"日志文件：{self.automation_manager.log_file}")
        log_hint.setWordWrap(True)
        log_hint.setStyleSheet(f"color: {COLORS['text_secondary']}; background: transparent; font-size: 12px; font-weight: 800;")
        log_layout.addWidget(log_hint)
        log_text = self._diagnostics_text_box("日志加载中...")
        log_layout.addWidget(log_text, 1)
        log_row = QHBoxLayout()
        log_row.addWidget(self._diagnostics_button("刷新日志", lambda: log_text.setPlainText(self.automation_manager.log_tail(12000) or "日志为空。")))
        log_row.addWidget(self._diagnostics_button("打开日志", self.open_automation_log_file))
        log_row.addWidget(self._diagnostics_button("复制日志路径", self.copy_automation_log_path))
        log_row.addStretch()
        log_layout.addLayout(log_row)
        tabs.addTab(log_tab, "日志")

        paths_tab = QWidget(dialog)
        paths_layout = QVBoxLayout(paths_tab)
        paths_layout.setContentsMargins(12, 12, 12, 12)
        paths_layout.setSpacing(10)
        paths_payload = {
            "plugin_dir": self.automation_manager.plugin_root,
            "backend_dir": self.automation_manager.backend_dir,
            "log_file": self.automation_manager.log_file,
            "base_url": self.automation_manager.base_url,
        }
        paths_text = self._diagnostics_text_box(json.dumps(paths_payload, ensure_ascii=False, indent=2), height=180)
        paths_layout.addWidget(paths_text)
        paths_row = QHBoxLayout()
        paths_row.addWidget(self._diagnostics_button("打开插件目录", self.open_automation_plugin_dir))
        paths_row.addWidget(self._diagnostics_button("复制插件目录路径", self.copy_automation_plugin_dir))
        paths_row.addWidget(self._diagnostics_button("复制全部路径", lambda: QApplication.clipboard().setText(paths_text.toPlainText())))
        paths_row.addStretch()
        paths_layout.addLayout(paths_row)
        paths_layout.addStretch()
        tabs.addTab(paths_tab, "目录")

        layout.addWidget(tabs, 1)

        tab_index = {"status": 0, "provider": 1, "log": 2, "paths": 3}.get(str(initial_tab or "status"), 0)
        tabs.setCurrentIndex(tab_index)

        footer = QHBoxLayout()
        footer.addStretch()
        close_btn = self._diagnostics_button("关闭", dialog.close)
        footer.addWidget(close_btn)
        layout.addLayout(footer)

        diagnostics_worker = DiagnosticsLoadWorker(self.automation_manager, self.effective_automation_model())
        dialog._diagnostics_worker = diagnostics_worker

        def apply_diagnostics_payload(data: dict):
            status_text.setPlainText(str(data.get("status_text") or "状态为空。"))
            log_text.setPlainText(str(data.get("log_text") or "日志为空。"))
            payload = data.get("provider_payload") if isinstance(data.get("provider_payload"), dict) else {}
            if payload:
                try:
                    provider_tabs = self._provider_info_tabs(payload, provider_tab)
                    index = provider_layout.indexOf(provider_placeholder)
                    provider_layout.removeWidget(provider_placeholder)
                    provider_placeholder.deleteLater()
                    provider_layout.insertWidget(max(0, index), provider_tabs, 1)
                except Exception as exc:
                    provider_placeholder.setPlainText(f"Provider 信息渲染失败：{exc}")
            else:
                errors = data.get("errors") if isinstance(data.get("errors"), list) else []
                provider_placeholder.setPlainText("\n".join(str(item) for item in errors) or "Provider 信息加载失败。")

        diagnostics_worker.finished_signal.connect(apply_diagnostics_payload)
        diagnostics_worker.finished_signal.connect(diagnostics_worker.deleteLater)
        QTimer.singleShot(0, diagnostics_worker.start)
        dialog.exec()
    ...
    def show_provider_info_dialog(self):
        ...
        title.setStyleSheet(f"color: {COLORS['text']}; background: transparent; font-size: 17px; font-weight: 900;")
        layout.addWidget(title)

        summary = QFrame(dialog)
        ...
        summary_items = [
            ("Base URL", str(payload.get("api_url") or "")),
            ("Model", str(payload.get("model") or "")),
            ("Status", str(payload.get("status") or "")),
            ("Web UI Search", "off" if payload.get("web_ui_search_enabled") is False else str(payload.get("web_ui_search_enabled"))),
        ]
        for index, (label, value) in enumerate(summary_items):
            label_widget = QLabel(label)
            label_widget.setStyleSheet(f"color: {COLORS['text_secondary']}; background: transparent; font-size: 11px; font-weight: 900;")
            value_widget = QLabel(value)
            value_widget.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)
            value_widget.setStyleSheet(f"color: {COLORS['text']}; background: transparent; font-size: 13px; font-weight: 900;")
            summary_layout.addWidget(label_widget, index // 2 * 2, index % 2)
            summary_layout.addWidget(value_widget, index // 2 * 2 + 1, index % 2)
        layout.addWidget(summary)

        layout.addWidget(self._provider_info_tabs(payload, dialog), 1)

        row = QHBoxLayout()
        row.addStretch()
        copy_btn = self._diagnostics_button("复制全部 JSON", lambda: QApplication.clipboard().setText(json.dumps(self.provider_info_payload(), ensure_ascii=False, indent=2)))
        close_btn = self._diagnostics_button("关闭", dialog.close)
    ...
    def run_web_fetch_responsive(self, url: str) -> str:
        if QApplication.instance() is None or QThread.currentThread() is not QApplication.instance().thread():
            self.automation_manager.start_provider()
            payload = self.automation_manager.request_json(
                "POST",
                "/v1/web-fetch",
                {
                    "url": url,
            ...
            summary = summarize_web_fetch_response(payload)
            if summary:
                return summary
            raise RuntimeError(f"provider 网页抓取返回格式异常: {payload}")
        ...
        thread = threading.Thread(target=_worker, name="AgentQtWebFetch", daemon=True)
        thread.start()
        while not bool(result.get("done")):
            QApplication.processEvents()
            time.sleep(0.03)
    ...
    def execute_terminal_extension_directives(self, directives: List[str]) -> str:
        ...
        web_fetch_urls = extract_web_fetch_urls("\n".join(cleaned_directives))
        ...
        if web_fetch_urls:
            ...
            max_workers = min(4, max(1, len(web_fetch_urls[:8])))
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_map = {
                    executor.submit(self.run_web_fetch_responsive, url): (idx, url)
                    for idx, url in enumerate(web_fetch_urls[:8])
                }
                ...
                for future in as_completed(future_map):
                    ...
                    try:
                        ...
                        ordered[idx] = f"网页抓取：{url}\n{summary}".strip()
                    except Exception as exc:
                        ordered[idx] = f"网页抓取：{url}\n抓取失败：{exc}".strip()
                fetch_outputs.extend(ordered[idx] for idx in sorted(ordered))
            output_parts.append("\n\n".join(part for part in fetch_outputs if part).strip())
    ...
    def start_web_research_extension_run(self, directives: List[str]):
        should_continue_automation = self.ensure_automation_loop_can_continue()
        cleaned_directives = [str(item or "").strip() for item in directives if str(item or "").strip()]
        ...
        def start_worker(long_retry_round: int = 0):
            ...
            def on_status(message: str):
                if self.web_research_worker is worker:
                    ...
                    if message:
                        ...
                        update_result_bubble(render_web_research_progress(), auto_follow=True)
            ...
            def on_finished(execution_text: str, error: str):
                ...
                if error and looks_like_provider_transient_error(error) and long_retry_round < PROVIDER_LONG_RETRY_ATTEMPTS:
                    ...
                    update_result_bubble(render_web_research_progress([retry_message]), auto_follow=True)
                    if self.automation_loop_active or should_continue_automation:
                        if should_continue_automation and not self.automation_loop_active:
                            self.automation_loop_active = True
                ...
                update_result_bubble(final_text_for_display, auto_follow=False)
    ...
    def start_web_fetch_extension_run(self, directives: List[str]):
        should_continue_automation = self.ensure_automation_loop_can_continue()
        cleaned_directives = [str(item or "").strip() for item in directives if str(item or "").strip()]
        urls = extract_web_fetch_urls("\n".join(cleaned_directives))
        if not cleaned_directives or not urls:
            return
        placeholder_lines = [
            "⏳ 网页抓取进行中...",
            "",
        ...
        progress_lines: List[str] = []
        result: Dict[str, object] = {"done": False, "execution_text": "", "error": ""}

        def render_web_fetch_progress(extra_lines: Optional[List[str]] = None) -> str:
            status_lines = progress_lines[-120:]
            parts = list(placeholder_lines)
            if status_lines or extra_lines:
                parts.extend(["", "Output:"])
                parts.extend(status_lines)
        ...
        def worker():
            try:
                ...
                ordered: Dict[int, str] = {}
                progress_lines.append(f"网页抓取任务开始：{len(targets)} 个网址，并发 {max_workers}。")
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    future_map = {}
                    for idx, url in enumerate(targets, start=1):
                        progress_lines.append(f"[{idx}/{len(targets)}] 排队抓取：{url}")
                        future = executor.submit(self.run_web_fetch_responsive, url)
                        future_map[future] = (idx, url)
                    for future in as_completed(future_map):
                        idx, url = future_map[future]
                        try:
                            summary = future.result()
                            progress_lines.append(f"[{idx}/{len(targets)}] 抓取完成：{url}")
                            ordered[idx] = f"网页抓取：{url}\n{summary}".strip()
                        except Exception as exc:
                            progress_lines.append(f"[{idx}/{len(targets)}] 抓取失败：{url} -> {exc}")
                            ordered[idx] = f"网页抓取：{url}\n抓取失败：{exc}".strip()
                execution_text = terminal_extension_execution_log(
                    cleaned_directives,
                    "\n\n".join(ordered[idx] for idx in sorted(ordered)).strip(),
                ).strip()
                result["execution_text"] = execution_text
            except Exception as exc:
                result["error"] = str(exc)
            finally:
                result["done"] = True

        thread = threading.Thread(target=worker, name="AgentQtWebFetchExtension", daemon=True)
        thread.start()

        def poll():
            if not bool(result.get("done")):
                update_result_bubble(render_web_fetch_progress(), auto_follow=True)
                QTimer.singleShot(80, poll)
                return
            final_text = str(result.get("execution_text") or "").strip()
            ...
            final_text_for_display = (
                final_text + "\n\n抓取过程日志：\n" + "\n".join(progress_lines[-120:])
            ).strip() if progress_lines else final_text
            update_result_bubble(final_text_for_display, auto_follow=False)
            context_content = build_execution_context_content(final_text, [])
            self.append_history({
                "type": "result",
            ...
            if self.automation_loop_active or should_continue_automation:
                if should_continue_automation and not self.automation_loop_active:
                    self.automation_loop_active = True
                self.set_status_bar_override("网页抓取已完成，正在衔接下一轮处理…", duration_ms=5000, mirror_to_terminal=False)
                QTimer.singleShot(80, lambda cc=context_content: self.schedule_next_automation_step(cc))
            elif self.automation_enabled:
                self.show_automation_composer(focus=False)
            else:
    ...
    def start_automation_worker(
        ...
        def actionable_automation_preview(text: str) -> bool:
            candidate = str(text or "").strip()
            if not candidate or looks_like_incomplete_plain_response(candidate):
                return False
            try:
                blocks = scan_all_code_blocks(candidate)
                commands = extract_bash_commands(candidate, blocks)
            except ValueError:
                return False
            if commands:
                return True
            directives = terminal_extension_directives_from_text(candidate)
            if not directives:
                return False
            stripped_candidate = strip_terminal_extension_directives_from_text(candidate)
        ...
        def start_chat_worker(messages: List[Dict[str, str]], long_retry_round: int = 0):
            ...
            def on_finished(text: str, error: str):
                ...
                if error:
                    ...
                    if partial_text and self.automation_loop_active and actionable_automation_preview(partial_text):
                        logger.warning(
                            "Automation worker received provider error after actionable preview; continuing execution error=%s chars=%d",
                            str(error or "")[:300],
                            len(partial_text),
                        )
                        self.automation_active_messages = []
                        self.automation_active_model = ""
                        preview_bubble = self.finalize_automation_preview_bubble(partial_text)
                        provider_io = copy.deepcopy(self.pending_provider_io)
                        self.pending_provider_io = None
                        self.handle_ai_response_text(partial_text, existing_bubble=preview_bubble, provider_io=provider_io)
                        return
                    if partial_text:
    ...
    def handle_ai_response_text(
        ...
        if self.automation_loop_active and self.wechat_active_request_id:
            ...
            elif web_fetch_urls and not has_real_command:
                self.set_status_bar_override("网页抓取进行中，正在等待结果…", duration_ms=15000, mirror_to_terminal=False)
                web_fetch_directives = list(terminal_extension_triggers)
        ...
        response_from_automation = bool(provider_io or existing_bubble is self.automation_preview_bubble)
        self.pending_execution_should_continue_automation = bool(
            self.automation_enabled
            and (self.automation_loop_active or response_from_automation)
            and self.ensure_automation_loop_can_continue()
        )

        self.worker = ExecuteWorker(commands, self.project_root)
        self.worker.output_signal.connect(self.on_output)
        self.worker.replace_output_signal.connect(self.on_output_replace)
    ...
    def on_output_replace(self, index: int, output: str):
        try:
            index = int(index)
        except (TypeError, ValueError):
            return
        while len(self.cmd_outputs) <= index:
            self.cmd_outputs.append("")
        self.cmd_outputs[index] = output
        self.cmd_output_render_dirty = True
        if not self.cmd_output_render_timer.isActive():
            self.cmd_output_render_timer.start(120)
    ...
    def on_finished(self, full_log: str):
        ...
        worker = self.worker
        self.worker = None
        should_continue_automation = bool(self.pending_execution_should_continue_automation)
        self.pending_execution_should_continue_automation = False
        if worker is not None:
            worker.deleteLater()
        ...
        if self.automation_enabled and (self.automation_loop_active or should_continue_automation):
            if should_continue_automation and not self.automation_loop_active:
                logger.warning("Automation execution finished with loop inactive; restoring continuation for local execution result.")
                self.automation_loop_active = True
    ...
    def ensure_automation_loop_can_continue(self, fallback_goal: str = "") -> bool:
        if not self.automation_enabled:
            return False
        if self.automation_loop_active:
            return True
        goal = str(self.automation_loop_goal or fallback_goal or "").strip()
        if not goal:
            for entry in reversed(self.history_entries):
                if str(entry.get("type") or "") == "prompt":
                    goal = self.prompt_text_from_system_prompt(str(entry.get("content") or "")).strip()
                    if goal:
                        break
        if not goal:
            return False
        self.automation_loop_active = True
        if self.automation_loop_round <= 0:
            self.automation_loop_round = 1
        self.automation_loop_goal = goal
        self.refresh_prompt_bubble_buttons()
        self.update_automation_composer_state()
        return True
    ...
    def schedule_next_automation_step(self, log_with_changes: str, skip_entry_id: str = "", attempt: int = 1):
        ...
        if self.is_execution_running() or self.is_automation_request_running():
            if attempt < 10:
                ...
                message = (
                    "下一轮自动续跑启动超时：系统检测到上一轮执行或模型请求状态仍未完全释放。"
                    "自动化循环已暂停，你可以稍后重试或手动继续。"
                )
                self.set_status_bar_override(message, duration_ms=10000)
                context_content = build_execution_context_content(
                    f"Automation paused:\n{message}",
                    [],
                )
                ...
                self.add_execution_result_entry(message, context_content=context_content)
                self.stop_automation_loop(message, ensure_manual_entry=True)
            return
