...
> 当前版本偏向“人工复制粘贴 + 本地安全执行”。后续可以把自动化 AI Provider 做成可选插件，而不是把 Playwright 等大依赖塞进主程序。
...
- 设置面板：字号、主题、语言等偏好。
- 自动化插件：可选下载 Playwright Provider，把“复制日志/等待 AI 回复”升级为自动交互。
- 更细的权限控制：执行前标记高风险命令。
- 更完整的跨平台打包：macOS、Windows、Linux 分别提供构建脚本。

## License

建议发布时补充一个开源许可证，例如 MIT License。
